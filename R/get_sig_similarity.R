...
#' from [maftools::compareSignatures()].
#'
...
get_sig_similarity = function(Signature, Ref=NULL, sig_db = "legacy", method = "cosine", verbose = TRUE)
{
  ...
  if (!all(rownames(w) %in% rownames(sigs))) {
    stop(paste0("The following components cannot be found in reference!\n",
  ...
  best_matches = lapply(1:nrow(corMat), function(i) {
    ...
    list(aetiology = ae, best_match = paste0("Best match: ",
                                             max_cor,
                                             " [similarity: ",
                                             max(corMat[i,]), "]"))
  })
  ...
  if (verbose) {
    ...
    for (i in 1:nrow(corMat)) {
      if (is.null(Ref)) {
        ...
        ae = paste0("Aetiology: ", ae, " [similarity: ",
                    max(corMat[i, ]), "]")
        message("--Found ", rownames(corMat)[i], " most similar to ",
                names(which(corMat[i, ] == max(corMat[i,]))), sep = "")
        message(paste0("   ", ae))
      } else {
        message("--Found ", rownames(corMat)[i], " most similar to ",
  ...
  invisible(list(similarity = corMat,
