# generate top data -------------------------------------------------------
inputdata <- readxl::read_xlsx(path = "D:/sv.xlsx",col_names = T)


# read sv -----------------------------------------------------------------
# imitate : maftools::read.maf()
read_sv <- function(input,
                    seg_cols = c("Sample", "chr1", "start1", "end1","chr2",
                                 "start2", "end2", "strand1", "stand2", "svclass")
  ...
  if (is.data.frame(x = input)) {
    ...
    input <- data.table::fread(file = input, sep = "\t",
                               data.table = TRUE, header = TRUE)
  }

  # check necessary name
  necessary.fields = c("sample", "chr1", "start1", "end1","chr2",
                       "start2", "end2", "strand1", "strand2", "svclass")

  # change column names to standard name
  for(i in 1:length(necessary.fields)){
    colFields = grep(
      pattern = paste0("^",necessary.fields[i]),
      x = colnames(input),
      ignore.case = T
    )
    if(length(colFields) != 0){
      colnames(input)[colFields] = necessary.fields[i]
    }
  }
  ...
  if(length(missing.fileds) > 0){
    ...
    stop(paste('missing required fields from SV:', missing.fileds)) #stop if any of required.fields are missing
  }
...
get_features_sv <- function(sv_data){
  ...
  res <- furrr::future_map(field, .get_feature, .progress = TRUE,
